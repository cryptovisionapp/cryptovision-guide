...
各取引分類について**損益計算**を解説しますが、上でも述べた通り、ブロックチェーン上のトランザクションにはガス代がかかるものが多いです。 各取引分類で解説する**損益計算**の値から、ガス代として消費したトークンの時価を費用として引き、そのトークンの取引損益=「取引時の時価-取得価格」を加算して、その結果がその取引の最終的な損益計算結果となります。
